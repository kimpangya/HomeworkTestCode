...
public class JwtUtil {
    ...
    public boolean validateToken(String token){
        try{
            ...
            log.error("Invlid JWT signature, 유효하지 않은 JWT 서명입니다.");
        }catch(ExpiredJwtException e){
            log.error("Expired JWT token, 만료된 JWT 토큰입니다.");
        }catch(UnsupportedJwtException e){
            log.error("Unsupported JWT token, 지원하지 않는 JWT 서명입니다.");
    ...
    public String createToken(String loginEmail){
        Date date = new Date(); // 발급일

        return BEARER_PREFIX +
                Jwts.builder()
                        .setSubject(loginEmail) //Subject에 로그인 이메일 넣음
